...
|---|---|---|---|---|
| 1 | **lighting** | dome 앰비언트 밝기 + blackbody 색온도 + overhead fixture(방향광·실제 그림자) + HDRI 환경맵 | `intensity`, `color_temperature`, `hdri`, `hdri_rotate`, **`hdri_normalize`**(+`_clamp`/`_ref`), `fixtures{kinds:[rect,distant],count,intensity,color_temperature,distance,elevation_deg,size}` | **HDRI 15종**(로컬 dir 오프라인/`isaac_skies` 온라인), fixture kinds **2**(rect·distant). `hdri_normalize: true` = 맵별 평균 복사량으로 dome intensity 보정(`median/L_map`, clamp 기본 `[0.3,3.0]`) → 풀이 12.6배 벌어져도 한 intensity 레인지로 커버. 로컬 파일만 측정 가능(원격 URL 은 gain 1.0) |
